...
export default function Header({ 
  ...
  const [activeDbKey, setActiveDbKey] = useState<DatabaseKey>(getActiveDatabaseKey());
  const [showDbModal, setShowDbModal] = useState(false);

  useEffect(() => {
    if (typeof window === 'undefined') return;

    const handleDbSwitched = () => {
      setActiveDbKey(getActiveDatabaseKey());
    };
    window.addEventListener('database_switched', handleDbSwitched);
    ...
    return () => {
      window.removeEventListener('database_switched', handleDbSwitched);
  ...
  return (
    <header className="bg-slate-900 text-white shadow-md border-b border-slate-800" id="main_header">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="flex items-center gap-1 sm:gap-2.5 shrink-0">
              ...
              {/* Database Switcher Button (Banco 01 vs Banco 02) */}
              <button
                type="button"
                onClick={() => setShowDbModal(true)}
                className={`flex items-center space-x-1 sm:space-x-1.5 px-2.5 py-1 rounded-full text-[10px] font-mono font-bold border transition-all duration-300 shadow-xs cursor-pointer hover:scale-105 active:scale-95 shrink-0 ${
                  activeDbKey === 'banco-01'
                    ? 'bg-blue-500/15 text-blue-400 border-blue-500/30 hover:bg-blue-500/25'
                    : 'bg-purple-500/15 text-purple-400 border-purple-500/30 hover:bg-purple-500/25'
                }`}
                title="Clique para alternar o Banco de Dados Ativo para todos os colaboradores"
              >
                <Folder className="h-3.5 w-3.5 text-amber-400 shrink-0" />
                <span className="uppercase tracking-wider text-[9px] hidden sm:inline whitespace-nowrap">
                  {activeDbKey === 'banco-01' ? 'Banco 01 (Ativo)' : 'Banco 02 (Ativo)'}
                </span>
                <span className="uppercase tracking-wider text-[9px] inline sm:hidden whitespace-nowrap">
                  {activeDbKey === 'banco-01' ? 'Banco 01' : 'Banco 02'}
                </span>
              </button>
      ...
      {/* Modal de Gerenciamento de Bancos de Dados (Banco 01 e Banco 02) */}
      {showDbModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-5 text-slate-800">
            <div className="flex items-start justify-between border-b border-slate-100 pb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Gerenciamento de Bancos Gratuitos Multi-Colaboradores
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-4">
              <div className="p-3.5 bg-blue-50 border border-blue-200 rounded-xl space-y-1.5 text-xs text-blue-900">
                ...
                <p className="text-blue-800 leading-relaxed text-[11px]">
                  Tudo o que foi feito no <strong>Banco 01</strong> (fechamentos de mapas, conferência do Gladson, vales e relatórios) continua <strong>totalmente preservado e visível no sistema</strong>! Ao alternar para o <strong>Banco 02</strong>, novos lançamentos serão gravados no Banco 02 enquanto a plataforma lê e consolida os dados de ambos os bancos simultaneamente em tempo real.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div
                  onClick={() => {
                    setActiveDatabaseKey('banco-01');
                    setActiveDbKey('banco-01');
                    alert("Banco de dados alterado para BANCO 01 com sucesso!\nNovos lançamentos serão gravados no Banco 01. Todos os dados permanecem visíveis.");
                  }}
                ...
                >
                  {activeDbKey === 'banco-01' && (
                    <span className="absolute top-2.5 right-2.5 bg-blue-600 text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                      Ativo no Momento
                    </span>
                  )}
                  <div className="flex items-center space-x-2 mb-2">
                    <Folder className={`h-5 w-5 ${activeDbKey === 'banco-01' ? 'text-blue-600' : 'text-slate-400'}`} />
                    <span className="font-extrabold text-sm text-slate-900">Banco 01</span>
                  </div>
                  <p className="font-mono text-[10px] text-slate-500 font-semibold truncate">
                    {BANCO_01_CONFIG.projectId}
                  </p>
                  <p className="text-[10px] text-slate-600 mt-2">
                    Banco Principal Inicial (Fechamentos de Mapas, Conferências)
                  </p>
                </div>
                ...
                <div
                  onClick={() => {
                    setActiveDatabaseKey('banco-02');
                    setActiveDbKey('banco-02');
                    alert("Banco de dados alterado para BANCO 02 com sucesso!\nNovos lançamentos serão gravados no Banco 02. Todos os dados anteriores do Banco 01 permanecem visíveis.");
                  }}
                  className={`p-4 rounded-xl border-2 transition-all cursor-pointer relative ${
                    activeDbKey === 'banco-02'
                      ? 'bg-purple-50/80 border-purple-600 shadow-sm'
                      : 'bg-slate-50 border-slate-200 hover:border-slate-300'
                  }`}
                >
                  {activeDbKey === 'banco-02' && (
                    <span className="absolute top-2.5 right-2.5 bg-purple-600 text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                      Ativo no Momento
                    </span>
                  )}
                  <div className="flex items-center space-x-2 mb-2">
                    <Folder className={`h-5 w-5 ${activeDbKey === 'banco-02' ? 'text-purple-600' : 'text-slate-400'}`} />
                    <span className="font-extrabold text-sm text-slate-900">Banco 02</span>
                  </div>
                  <p className="font-mono text-[10px] text-slate-500 font-semibold truncate">
                    {BANCO_02_CONFIG.projectId}
                  </p>
                  <p className="text-[10px] text-slate-600 mt-2">
                    Banco Auxiliar para Expansão da Cota Gratuita
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
              <span className="text-[10px] text-slate-500 font-medium">
                Seleção sincronizada no navegador dos colaboradores.
              </span>
              <button
                onClick={() => setShowDbModal(false)}
